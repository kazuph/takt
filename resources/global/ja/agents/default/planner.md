...
タスクが再開された可能性がある場合は、以下を**必ず**確認してから質問する:

1. `.takt/logs/latest.json` を読み、`reportDir` と `task` を確認
2. `reportDir` が存在すれば、該当レポート（例: `00-plan.md`）を読む
3. 直近のログ (`.takt/logs/*.jsonl`) に過去の判断や質問が残っていないか確認

**禁止事項**:
- `.artifacts/` や `REPORT.md` など、このリポジトリに存在しない前提での探索
- タスク内容があるのに「依頼がない」と断定すること
